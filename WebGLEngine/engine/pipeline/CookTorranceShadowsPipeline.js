...
class CookTorranceShadowsPipeline extends Pipeline {
    ...
    constructor(gl){
        ...
        var fragmentShaderSrc = `#version 300 es
            ...
            /**
            * Compute the specular lighting with Cook-Torrance technique.
            ...
            * @param matColor Color material
            */
            vec4 DoCookTorrance(vec3 light, vec3 normal, vec3 viewDirection, vec4 lightColor, vec4 matColor)
            {
                vec3 vHalf = normalize(light + normalize(viewDirection));
                ...
                float normalDotCamera = dot(vNormal, viewDirection);
                float F = mix(pow(1.0f - normalDotCamera, 5.0f), 1.0f, mat.fresnel);

                //---------- Compute self shadowing term G ---------
                float normalDotLight = dot(vNormal, light);
                float X = 2.0f * normalDotHalf / dot(viewDirection, vHalf);
                float G = min(1.0f, min(X * normalDotLight, X * normalDotCamera));

                //---------- Compute final Cook-Torrance specular term ---------
                float pi = 3.1415926535897932384626433832f;
                float cookTorrance = (D * F * G) / (normalDotCamera * pi);

                return lightColor * max(0.0f, cookTorrance) * lightColor.a * matColor;
            }
            ...
            Lighting ComputeDirectionalLight(DirectionalLight dl, Material material, vec3 normal, vec3 viewDirection)
            {
                ...
                lighting.specular = dl.intensity * DoCookTorrance(light.xyz, normalize(normal), viewDirection, dl.color, material.specularColor);
            ...
            float ShadowCalculation(vec4 fragPosLightSpace)
            {
                ...
                vec3 lightPos = vec4(u_mView * dl.position).xyz;
